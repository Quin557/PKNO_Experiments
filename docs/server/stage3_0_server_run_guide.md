# Stage3_0 Param-KNO 服务器运行指南

本文件用于直接启动 Stage3_0，不修改旧的 `server_run_checklist.md`。

## 1. 阶段目录

```bash
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
RESULT_DIR="results/$STAGE"
REPORT_DIR="reports/$STAGE"

mkdir -p "$LOG_DIR" "$OUT_DIR" "$RESULT_DIR" "$REPORT_DIR"
```

每个 run 输出：

```text
outputs/stage3_0_param_kno/<run_name>/
  args.json
  config.yaml
  env.txt
  metrics.csv
  rollout_error_by_step.csv
  spectral_metrics.csv
```

## 2. 环境

优先复用 Stage 0 已经能跑通 KNO 的 PyTorch 环境。

```bash
conda activate <your_torch_env>
pip install -r requirements.txt
```

检查：

```bash
python - <<'PY'
import torch, numpy, scipy, h5py, yaml
print("torch:", torch.__version__)
print("cuda:", torch.version.cuda)
print("cuda available:", torch.cuda.is_available())
print("visible gpu count:", torch.cuda.device_count())
if torch.cuda.is_available():
    print("device 0:", torch.cuda.get_device_name(0))
PY
```

Stage3_0 是 PyTorch-only，不需要安装 TensorFlow。

## 3. 数据路径

```bash
source configs/data_paths.env

ls -lh "$DATA_ROOT/$BURGERS_FILE"
ls -lh "$DATA_ROOT/$NS2D_V1E3_FILE"
ls -lh "$DATA_ROOT/$NS2D_V1E4_FILE"
ls -lh "$DATA_ROOT/$SHALLOW_WATER_FILE"
```

## 4. Smoke Tests

先每个数据集跑 1 epoch。目的只是确认数据读取、condition vector、complex Koopman einsum、rollout 写文件正常。

### Burgers

```bash
source configs/data_paths.env
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
mkdir -p "$LOG_DIR" "$OUT_DIR" "results/$STAGE" "reports/$STAGE"

export CUDA_VISIBLE_DEVICES=0
RUN=smoke_pkno_burgers_o32_m16_r8_seed42

nohup python -u experiments/stage3_0/train_pkno_burgers.py \
  --data-path "$DATA_ROOT/$BURGERS_FILE" \
  --run-name "$RUN" \
  --output-dir "$OUT_DIR" \
  --epochs 1 \
  --batch-size 64 \
  --sub 32 \
  --operator-size 32 \
  --modes 16 \
  --decompose 8 \
  --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &

echo $!
```

### NS v1e-3

```bash
source configs/data_paths.env
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
mkdir -p "$LOG_DIR" "$OUT_DIR" "results/$STAGE" "reports/$STAGE"

export CUDA_VISIBLE_DEVICES=0
RUN=smoke_pkno_ns_v1e3_o32_m16_r8_t40_seed42

nohup python -u experiments/stage3_0/train_pkno_ns_v1e3.py \
  --data-path "$DATA_ROOT/$NS2D_V1E3_FILE" \
  --run-name "$RUN" \
  --output-dir "$OUT_DIR" \
  --epochs 1 \
  --batch-size 10 \
  --t-in 10 \
  --t-out 40 \
  --ntrain 1000 \
  --ntest 200 \
  --operator-size 32 \
  --modes 16 \
  --decompose 8 \
  --max-grad-norm 1.0 \
  --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &

echo $!
```

### NS v1e-4

```bash
source configs/data_paths.env
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
mkdir -p "$LOG_DIR" "$OUT_DIR" "results/$STAGE" "reports/$STAGE"

export CUDA_VISIBLE_DEVICES=0
RUN=smoke_pkno_ns_v1e4_o32_m16_r8_t20_seed42

nohup python -u experiments/stage3_0/train_pkno_ns_v1e4.py \
  --data-path "$DATA_ROOT/$NS2D_V1E4_FILE" \
  --run-name "$RUN" \
  --output-dir "$OUT_DIR" \
  --epochs 1 \
  --batch-size 10 \
  --t-in 10 \
  --t-out 20 \
  --ntrain 1000 \
  --ntest 200 \
  --operator-size 32 \
  --modes 16 \
  --decompose 8 \
  --max-grad-norm 1.0 \
  --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &

echo $!
```

### Shallow-water

```bash
source configs/data_paths.env
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
mkdir -p "$LOG_DIR" "$OUT_DIR" "results/$STAGE" "reports/$STAGE"

export CUDA_VISIBLE_DEVICES=0
RUN=smoke_pkno_shallow_water_o32_m16_r8_t40_seed42

nohup python -u experiments/stage3_0/train_pkno_shallow_water.py \
  --data-path "$DATA_ROOT/$SHALLOW_WATER_FILE" \
  --run-name "$RUN" \
  --output-dir "$OUT_DIR" \
  --epochs 1 \
  --batch-size 5 \
  --t-in 10 \
  --t-out 40 \
  --ntrain 900 \
  --ntest 100 \
  --operator-size 32 \
  --modes 16 \
  --decompose 8 \
  --dt 0.01 \
  --lr 2e-4 \
  --delta-scale 0.02 \
  --max-grad-norm 0.5 \
  --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &

echo $!
```

## 5. Full Runs

Smoke 通过后再跑 full。若 GPU 足够，可以一张卡跑一个实验。

```bash
source configs/data_paths.env
STAGE=stage3_0_param_kno
LOG_DIR="logs/$STAGE"
OUT_DIR="outputs/$STAGE"
mkdir -p "$LOG_DIR" "$OUT_DIR" "results/$STAGE" "reports/$STAGE"
```

### Burgers full

```bash
RUN=pkno_burgers_o32_m16_r8_ep500_seed42
CUDA_VISIBLE_DEVICES=0 nohup python -u experiments/stage3_0/train_pkno_burgers.py \
  --data-path "$DATA_ROOT/$BURGERS_FILE" \
  --run-name "$RUN" --output-dir "$OUT_DIR" \
  --epochs 500 --batch-size 64 --sub 32 \
  --operator-size 32 --modes 16 --decompose 8 \
  --lr 1e-3 --seed 42 --save-checkpoint --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &
echo $!
```

### NS v1e-3 full

```bash
RUN=pkno_ns_v1e3_o32_m16_r8_t40_ep500_seed42
CUDA_VISIBLE_DEVICES=1 nohup python -u experiments/stage3_0/train_pkno_ns_v1e3.py \
  --data-path "$DATA_ROOT/$NS2D_V1E3_FILE" \
  --run-name "$RUN" --output-dir "$OUT_DIR" \
  --epochs 500 --batch-size 10 --t-in 10 --t-out 40 \
  --ntrain 1000 --ntest 200 \
  --operator-size 32 --modes 16 --decompose 8 \
  --lr 5e-4 --max-grad-norm 1.0 --seed 42 --save-checkpoint --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &
echo $!
```

### NS v1e-4 full

```bash
RUN=pkno_ns_v1e4_o32_m16_r8_t40_ep500_seed42
CUDA_VISIBLE_DEVICES=4 nohup python -u experiments/stage3_0/train_pkno_ns_v1e4.py \
  --data-path "$DATA_ROOT/$NS2D_V1E4_FILE" \
  --run-name "$RUN" --output-dir "$OUT_DIR" \
  --epochs 500 --batch-size 10 --t-in 10 --t-out 40 \
  --ntrain 1000 --ntest 200 \
  --operator-size 32 --modes 16 --decompose 8 \
  --lr 5e-4 --max-grad-norm 1.0 --seed 42 --save-checkpoint --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &
echo $!
```

### Shallow-water full

```bash
RUN=pkno_shallow_water_o32_m16_r8_t40_ep500_seed42
CUDA_VISIBLE_DEVICES=3 nohup python -u experiments/stage3_0/train_pkno_shallow_water.py \
  --data-path "$DATA_ROOT/$SHALLOW_WATER_FILE" \
  --run-name "$RUN" --output-dir "$OUT_DIR" \
  --epochs 500 --batch-size 5 --t-in 10 --t-out 40 \
  --ntrain 900 --ntest 100 \
  --operator-size 32 --modes 16 --decompose 8 \
  --dt 0.01 --lr 2e-4 --delta-scale 0.02 --max-grad-norm 0.5 \
  --seed 42 --save-checkpoint --device cuda \
  > "$LOG_DIR/$RUN.log" 2>&1 &
echo $!
```

## 6. 监控

```bash
watch -n 5 nvidia-smi
tail -f logs/stage3_0_param_kno/<run_name>.log
tail -n 5 outputs/stage3_0_param_kno/<run_name>/metrics.csv
cat outputs/stage3_0_param_kno/<run_name>/spectral_metrics.csv
```

## 7. 汇总

```bash
python scripts/collect_results.py
cat results/stage3_0_param_kno/run_summary.csv
cat results/run_summary.csv
```

## 8. 推荐顺序

```text
1. Burgers smoke
2. NS v1e-4 smoke
3. NS v1e-3 smoke
4. Shallow-water smoke
5. Burgers full
6. NS v1e-4 full
7. NS v1e-3 full
8. Shallow-water full
```

NS v1e-4 默认 `t_out=20`，因为该文件通常只有 30 个时间步；v1e-3 和 shallow-water 默认 `t_out=40`。
