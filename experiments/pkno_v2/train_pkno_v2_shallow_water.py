...
def main() -> None:
    p = argparse.ArgumentParser("Train PKNO_v2-A on shallow-water"); p.add_argument("--data-path", type=Path, required=True); p.add_argument("--t-in", type=int, default=10); p.add_argument("--t-out", type=int, default=40); p.add_argument("--sub", type=int, default=1); p.add_argument("--dt", type=float, default=0.01); p.add_argument("--split-mode", choices=["tuning", "final"], default="final"); add_common_args(p); p.set_defaults(batch_size=5, lr=5e-5, basis_kind="shallow_water", decompose=4, delta_scale=0.005, max_grad_norm=0.1)
